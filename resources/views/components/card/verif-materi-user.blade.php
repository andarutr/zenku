    ...
    <div class="col-lg-3 mb-3">
        <div class="card shadow" style="border-radius: 25px">
            ...
            <div class="card-body">
                ...
                <h5 class="card-title">
                    <a href="{{ route('user.materi.show', ['id_card' => $mat->id, 'url_card' => $mat->url_card]) }}" style="text-decoration: none;">{{ $mat->title_card  }}</a>
                ...
                @endphp
                <a href="{{ route('user.like.store', ['id_card' => $mat->id]) }}" class="btn btn-md"><i class="fas fa-heart 
                    @isset($likes)
                    text-danger
                    @endisset"></i></a>
                
                <a href="{{ route('user.materi.show', ['id_card' => $mat->id, 'url_card' => $mat->url_card]) }}" class="btn btn-md"><i class="fas fa-comment text-secondary"></i></a>
                <a href="{{ route('user.materi.show', ['id_card' => $mat->id, 'url_card' => $mat->url_card]) }}" class="btn btn-primary"><i class="fas fa-book-reader"></i> Belajar</a>
            </div>
        </div>
    </div>
@endforeach