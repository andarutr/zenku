<div>
    <form action="{{ route('admin.role.store') }}" method="POST">@csrf
        <div class="form-group">
            ...
            <input type="text" class="form-control" name="name_role" id="name_role" value="{{ old('name_role') }}">
            @error('name_role')<p class="text-danger">{{ $message }}</p>@enderror
        </div>
        <div class="form-group">
            <label for="id_role">ID Role</label>
            <input type="number" class="form-control" name="id_role" id="id_role" value="{{ old('id_role') }}">
