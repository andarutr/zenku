...
Route::middleware('isUser')->prefix('/user')->name('user.')->group(function(){
	...
	Route::delete('/like/{id}', [Like::class, 'destroy'])->name('like.destroy');
	Route::get('/like/materi/{id}', [Like::class, 'store'])->name('like.store');
	...
	Route::get('/materi/{id}/{url}', [Materi::class, 'show'])->name('materi.show');
	Route::prefix('comment')->group(function(){
		Route::get('/', [Comment::class, 'index'])->name('comment.index');
		Route::post('/store/{id}', [Comment::class, 'store'])->name('comment.store');
		Route::get('/{id}/edit', [Comment::class, 'edit'])->name('comment.edit');
		Route::put('/{id}', [Comment::class, 'update'])->name('comment.update');
		Route::delete('/{id}', [Comment::class, 'destroy'])->name('comment.destroy');
	});
